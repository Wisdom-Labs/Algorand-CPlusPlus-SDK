...
provider_application_info_get(uint64_t app_id, app_values_t *global_states)
{
    ...
    int ret = sprintf(relative_path, "/v2/applications/%llu", (long long unsigned int)app_id);      // bug fixing
...
provider_ping(void)
{
    ...
    if (response_code >= 300)
    {
        return (ret_code_t) ( VTC_ERROR_HTTP_BASE + response_code );  // bug fixing
